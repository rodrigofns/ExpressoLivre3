...
function prepara_feriados_nacionais()
{
  ...
  var aux1=(19 * a) + 24;
  var d= aux1 % 30;
  var aux1= (2 * b) + (4 * c) + (6 * d) + 5;
  var e = aux1 % 7;
  var dia_pascoa = 22 + d + e;
  var mes_pascoa = 2;
  ...
  var pascoa =  new Date(ano,mes_pascoa,dia_pascoa ); 
  var auxiliar = parseInt(pascoa.getTime()) - (47 * 1000 * 60 * 60 * 24);
  var carnaval = new Date(auxiliar);
  var auxiliar = parseInt(pascoa.getTime()) - (2 * 1000 * 60 * 60 * 24);
  var sexta_santa = new Date(auxiliar);
  var auxiliar = parseInt(pascoa.getTime()) + (60 * 1000 * 60 * 60 * 24);
  var corpus = new Date(auxiliar);
  
  // Os tres feriados aseguir dependem da pascoa...
  if(mes == (pascoa.getMonth()+1) )
     {
        feriados[pascoa.getDate()] = 'Domingo de Pascoa';
	feriados[sexta_santa.getDate()] = 'Sexta-feira Santa';
     }
  if(mes == (carnaval.getMonth()+1) ) 
     {
        feriados[carnaval.getDate()] = 'Carnaval';
     }     
  if(mes == (corpus.getMonth()+1) )
     {
        feriados[corpus.getDate()] = 'Corpus Christi';
     }          
...
//Outros feriados Nacionais
    switch (mes*1) 
	{
	    case 1: //  Primeiro de Janeiro:
                 feriados[1]='Dia Mundial da Paz';
		 break;
	    case 4: //  Tiradentes:
                 feriados[21]='Tiradentes';
		 break;
	    case 5: //   Dia do trabalhador:
                 feriados[1]='Dia do Trabalhador';
		 break;
	    case 9: //   Indepndencia do Brasil:
                 feriados[7]='Indepndencia do Brasil';
		 break;
	    case 10: //   Nossa Senhora Aparecida:
                 feriados[12]='Nossa Senhora Aparecida';
		 break;
	    case 11: //   Finados/ Republica:
                 feriados[2]='Finados';
		 feriados[15]='Proclamação da Republica';
		 break;
	    case 12: //   Natal:
                 feriados[25]='Natal';
		 break;		 
...
function prepara_feriados(vAno,vMes)
{
ano = vAno;
mes = vMes;
// prepara array para feriados ...
for(var i=1;i<32;i++)
  {
    feriados[i] = '';
  }
//  Feriados nacionais ...
prepara_feriados_nacionais();
}
 ...
 */
/**
 * @class Tine.Calendar.DatePickerHolliday
 ...
 * @xtype datepicker
 */
Tine.Calendar.DatePickerHolliday = Ext.extend(Ext.BoxComponent, {
    /**
    ...
    initComponent : function(){
        Tine.Calendar.DatePickerHolliday.superclass.initComponent.call(this);
    ...
    onEnable: function(initial){
        Tine.Calendar.DatePickerHolliday.superclass.onEnable.call(this);    
    ...
    // private
    onDisable : function(){
        Tine.Calendar.DatePickerHolliday.superclass.onDisable.call(this);   
    ...
    update : function(date, forceRefresh){
        if(this.rendered){
                prepara_feriados(date.getFullYear().toString(), (date.getMonth()+1).toString());
	        ...
	        //}
                for(; i < days; i++){
	            var intDay = i - startingPos + 1;
                    var auxDate = new Date((date.getMonth()+1).toString() + "/" + (intDay) + "/" + date.getFullYear().toString());
                    textEls[i].innerHTML = (intDay);
                    if(feriados[intDay] != '') {
                        textEls[i].innerHTML = '<FONT COLOR="#FF3300"><b>' + (intDay) + '</b></FONT>';
                        textEls[i].title = feriados[intDay];
                    }
                    if(auxDate.getDay() == '0') {
...
Ext.reg('datepicker', Tine.Calendar.DatePickerHolliday);
