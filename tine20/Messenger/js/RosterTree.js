...
Tine.Messenger.RosterTree = function(iq){
    ...
    var buddyContext = function(_node, e){
            ...
            items.push({text: app.i18n._('Rename'),
                          handler: renameBuddy,
                          node:_node,
                          icon: '/images/messenger/user_edit.png'});
            items.push({text: app.i18n._('Remove'), 
                          handler: removeBuddy,
                          node:_node, 
            ...
            if( _node.attributes.subscription == "none" || 
                ...
                _node.attributes.subscription == "from")
                    items.push({text: app.i18n._('Subscribe'), 
                                  handler: subscribeBuddy,
                                  node:_node,
                                  icon:""});
            items.push({text: app.i18n._('Move to'),
                          node:_node,
                          listeners: {
                              render: function(e){
    ...
    var renameBuddy = function(_node, e){
            var treeEditor = new Ext.tree.TreeEditor(Ext.getCmp('messenger-roster'), {
                    ...
                    allowBlank:false,
                    blankText:app.i18n._('A name is required'),
                    selectOnFocus:true
            });
    ...
    var removeBuddy = function(_node, e){
        ...
        Ext.Msg.buttonText.yes = app.i18n._('Yes');
    ...
    var buildSubMenuGrpItems = function(e){
        ...
        for(var i=0; i < groups.length; i++){
            ...
            if(group != user_group){
                ...
                items.push({text: app.i18n._('Empty'),
                            disabled: true 
                        });
            }
        }
        if(user_group != app.i18n._(NO_GROUP)){
            items.push(new Ext.menu.Separator());
             items.push({text: app.i18n._(NO_GROUP),
                         handler: function(choice, ev){
                            Tine.Messenger.RosterHandler.moveContactFromGroups(jid, choice.text);
                        }
                    });
    ...
    var renameGroupAction = function(_e, name){
        if(name && name != _e.startValue){
            if(groupExist(name)){
                Ext.Msg.alert(app.i18n._('Rename Group'),
                                app.i18n._('The group already exists!'));
                return false;
            } else {
                return true;
            }
                ...
                if(jid.length > 0){
                    ...
                    var _buddy = new Ext.tree.TreeNode({ //buddy adden
                                    ...
                                    qtip: "JID : "+jid+"<br>"
                                            ...
                                            +app.i18n._('Subscription')+" : "+subscription
                    });
                    ...
                    if($(this).children("group").text().trim().length > 0){
                        ...
                        for(i=0; i < rootNode.childNodes.length; i++){
                            if(rootNode.childNodes[i].text == app.i18n._(NO_GROUP)){
                                hasGroupNoGroup = true;
                                node = i;
                            }
                        }
                        if(!hasGroupNoGroup){
                            Tine.Messenger.RosterTree().addGroup(app.i18n._(NO_GROUP));
        ...
        $(xml).find("group").each(function(){
            ...
            if(_group_name.trim() != ''){
                if($.inArray(_group_name, _arr_groups) === -1){
                    ...
                    if(_group_name != app.i18n._(NO_GROUP)){
...
//        _chat.on("contextmenu", buddyContext);
    ...
    var addOrderedOnTreeNodeLevel = function(node, parentNode){
        ...
        if(v_nodes.length == 0 || node.text == app.i18n._(NO_GROUP)){
            parentNode.appendChild(node);
        } else {
            var node_not_inserted = true;
            for(var i=0; i < v_nodes.length && node_not_inserted; i++){
                if(node.text < v_nodes[i].text || v_nodes[i].text == app.i18n._(NO_GROUP)){
